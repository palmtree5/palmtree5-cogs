...
class FEUnitPicker():
    ...
    def __init__(self, bot):
        self.bot = bot
        self.settings = dataIO.load_json("data/feunitpicker/settings.json")

    @commands.command(pass_context=True)
    async def fepick(self, ctx, game: str):
    ...
    async def fe_draft_generator(self, ctx, game, use_pms=True):
        ...
        game_data = dataIO.load_json("data/feunitpicker/" + self.settings[game])
        if use_pms:
    ...
    def generate_draft(self, draft_settings):
        ...
        data = dataIO.load_json("data/feunitpicker/" + self.settings[draft_settings["game"]])
